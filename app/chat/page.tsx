...
export default function ChatPage() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Real AI analysis using our backend
      ...
      if (result.success && result.analysis) {
        ...
        if (currentInput.toLowerCase().includes("analyze")) {
          botResponse = `I've analyzed your text using ${result.metadata?.processing_model || "TruthGuard AI"}. Here's what I found:
...
- Source Quality: ${((analysis.credibility_assessment?.source_quality || 0) * 100).toFixed(1)}%
- Logical Consistency: ${((analysis.credibility_assessment?.logical_consistency || 0) * 100).toFixed(1)}%

**🤖 Processing Info:**
- Model: ${analysis.model_version || result.metadata?.processing_model || "TruthGuard AI"}
- Confidence: ${((analysis.confidence || 0) * 100).toFixed(1)}%
- MongoDB Storage: ${result.analysis.mongodb_stored ? "✅ Stored" : "❌ Not stored"}
...
What would you like to explore first?`
        ...
        const botMessage: Message = {
          ...
          content: botResponse,
          timestamp: new Date(),
          analysis: currentInput.toLowerCase().includes("analyze")
            ? {
                ...
                processing_model: analysis.model_version || result.metadata?.processing_model || "TruthGuard AI",
              }
            : undefined,
        }
                              ...
                              <div className="space-y-1 text-xs">
                                <div
                                  className={`flex justify-between ${
                                    message.type === "user" ? "text-white" : getBiasColor(message.analysis.bias_score)
                                  }`}
                                >
                                ...
                                <div
                                  className={`flex justify-between ${
                                    message.type === "user"
                                      ? "text-white"
                                      : getRiskColor(message.analysis.misinformation_risk)
                                  }`}
                                >
