...
export default function RealTimePage() {
    ...
    useEffect(() => {
        ...
        // This EventSource connects to the frontend's /api/realtime, which then fetches from backend
    ...
    const triggerNewsScraping = async () => {
        try {
            ...
            if (result.success) {
                ...
                alert("Failed to trigger news scraping: " + (result.details || "Unknown error"));
            }
        } catch (error) {
            console.error("Failed to trigger news scraping:", error)
            alert("Error triggering news scraping. Check console for details.");
        }
    }
                        ...
                        <CardContent>
                            <Tabs defaultValue="all" className="w-full">
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex items-center justify-between mb-2">
                                                            <h4 className="font-medium">
                                                                ...
                                                                {update.type === "news_scraping_initiated" && "News Scraping Initiated"}
                                ...
                                <TabsContent value="processing" className="mt-4">
                                    <div className="space-y-3 max-h-96 overflow-y-auto">
                                        {updates
                                            .filter((u) => u.type === "comprehensive_update" || u.type === "news_scraping" || u.type === "news_scraping_initiated")
                                            .map((update, index) => (
                                                <div key={index} className="p-4 border rounded-lg">
                                                    <div className="flex items-center justify-between mb-2">
                            <span className="font-medium">
                              {update.type === "news_scraping" ? "News Scraping Complete" : update.type === "news_scraping_initiated" ? "News Scraping Initiated" : "Processing Batch"}
                            </span>
                                                        <span className="text-xs text-gray-500">
                              {new Date(update.timestamp).toLocaleTimeString()}
                            </span>
