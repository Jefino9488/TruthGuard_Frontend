...
export default function MongoDBShowcasePage() {
  ...
  const fetchAnalytics = async () => {
    try {
      // Fetch analytics from the frontend API which now routes to backend's dashboard-analytics
      const response = await fetch("/api/mongodb-analytics?type=overview") // Request overview type
      const data = await response.json()
      if (data.success) {
        setAnalytics(data.data) // data.data will be the raw dashboard analytics from backend
      }
    } catch (error) {
  ...
  const performVectorSearch = async () => {
    ...
    setLoading(true)
    try {
      // Call the frontend API for vector search which routes to backend's /vector-search
      const response = await fetch(`/api/mongodb-vector?q=${encodeURIComponent(searchQuery)}&limit=5`, {
        method: "GET", // Frontend's mongodb-vector GET for search
      });
      const data = await response.json()
      if (data.success) {
        setVectorSearchResults(data.data) // data.data contains the articles from backend's vector search
      } else {
        console.error("Vector search API returned an error:", data.error)
      }
    } catch (error) {
  ...
  // Extract data for charts from the `analytics` state, which holds the backend's dashboard analytics
  const biasDistributionData = analytics?.biasDistribution?.map((item: any, index: number) => ({
    name: `Bias ${item._id === 0 ? "Low (0-0.3)" : item._id === 0.3 ? "Medium (0.3-0.6)" : "High (0.6-1.0)"}`,
    value: item.count,
    fill: COLORS[index % COLORS.length],
  })) || [];

  const biasTrendsData = analytics?.biasTrends || []; // This would come from a specific backend endpoint for trends

  const sourceAnalysisData = analytics?.sourceComparison?.slice(0, 6).map((source: any) => ({
    _id: source._id,
    articleCount: source.articleCount,
    avgBias: source.averageBias,
    avgCredibility: source.averageCredibility,
    avgMisinfoRisk: source.averageMisinformationRisk,
  })) || [];

  const topicTrendsData = analytics?.topicTrends?.slice(0, 6) || []; // This would come from a specific backend endpoint for topics
                        ...
                        <div key={index} className="p-4 border rounded-lg bg-gray-50">
                          ...
                          <div className="grid grid-cols-3 gap-4 text-sm">
                            <div>
                              ...
                              <Progress value={(article.bias_score || 0) * 100} className="h-2 mt-1" />
                            </div>
                            <div>
                              <span className="font-medium">Credibility:</span>
                              <Progress value={(article.credibility_score || 0) * 100} className="h-2 mt-1" />
                            </div>
                            <div>
                              <span className="font-medium">Risk Level:</span>
                              <Progress value={(article.misinformation_risk || 0) * 100} className="h-2 mt-1" />
                            </div>
                          </div>
                        </div>
                    ))}
          ...
          <Tabs defaultValue="overview" className="w-full">
                      ...
                      <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                          <Pie
                              ...
                              dataKey="value"
                          >
                            {biasDistributionData.map((_: any, index: number) => (
            ...
            <TabsContent value="bias" className="mt-6">
              <Card>
                ...
                <CardContent>
                  <div className="h-80">
                    {biasTrendsData.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={biasTrendsData}>
                        ...
                        <div className="text-center text-gray-500 py-4">No bias trend data available from backend.</div>
                    )}
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {sourceAnalysisData.length > 0 ? (
                        sourceAnalysisData.map((source: any, index: number) => (
                        ...
                        <div className="text-center text-gray-500 py-4">No source comparison data available from backend.</div>
                    )}
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {topicTrendsData.length > 0 ? (
                        topicTrendsData.map((topic: any, index: number) => (
                            <div key={index} className="p-4 border rounded-lg">
                        ...
                        <div className="text-center text-gray-500 py-4">No topic analysis data available from backend.</div>
                    )}
                  </div>
                </CardContent>
              </Card>
